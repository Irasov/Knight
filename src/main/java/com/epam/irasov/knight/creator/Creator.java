        ...
        for (int i = 1; i <= this.countAmmunition; i++) {
            ...
            switch (randName) {
                case ARMOR:
                    ...
                    break;
                case HELMET:
                    randWeight = (RANDOM.nextInt(HELMET_WEIGHT) + ADDITIVE_NOT_RANDOM_ZERO);
                    randPrice = (RANDOM.nextInt(PRICE_HELMET) + ADDITIVE_PRICE);
                    componentAmmunition = new Helmet(randName, randomHelmetType(), randWeight, randPrice, randomConstBalaclava());
                    knight.addAmmunition(componentAmmunition);
                    break;
                case SHIELD:
                    randWeight = (RANDOM.nextInt(SHIELD_WEIGHT) + ADDITIVE_NOT_RANDOM_ZERO);
                    randPrice = (RANDOM.nextInt(PRICE_SHIELD) + ADDITIVE_PRICE);
                    componentAmmunition = new Shield(randName, randomShieldType(), randWeight, randPrice, randomShieldMaterial());
                    knight.addAmmunition(componentAmmunition);
                    break;
                case MELLE_WEAPON:
                    randWeight = (RANDOM.nextInt(WEAPON_WEIGHT) + ADDITIVE_NOT_RANDOM_ZERO);
                    randPrice = (RANDOM.nextInt(PRICE_WEAPON) + ADDITIVE_PRICE);
